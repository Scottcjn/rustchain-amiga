 ...
 *
 * VENDORED COPY for gemini/ (2026-07-03), copied verbatim from
 * claude/client/vendor/rtc_common.c (itself vendored from
 * rustchain-amiga/tools/common/rtc_common.c). Nothing changed here: the
 * gemini client only uses rtc_net_open()/rtc_net_close() (bsdsocket v3,
 * which closes cleanly on exit -- see the comment in rtc_net_open below)
 * and rtc_check_break(). It has its own URL parser (gemini:// not http://),
 * its own status-line parser (Gemini's "NN meta\r\n", not an HTTP header
 * block), and its own TCP connect + AmiSSL transport in gemini.c, so the
 * HTTP/JSON/SHA-1 helpers in this file go unused there. Kept whole rather
 * than trimmed so this stays a faithful, diffable copy of the origin.
 */
